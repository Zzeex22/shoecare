...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-3xl shadow-lg border-2 border-shoeBlue/20 overflow-hidden relative">
        ...
        <div class="p-8">
            @php
                $step = 1;
                if($row->status == 'Menunggu Pickup') $step = 2;
                if($row->status == 'Sedang Dicuci') $step = 3;
                
                // CEK APAKAH INI BELI PRODUK ATAU JASA CUCI
                $isProduct = str_starts_with($row->kode_order, 'PRD');
            @endphp
            <div class="flex items-center justify-between relative max-w-4xl mx-auto">
                ...
                <div class="relative z-10 flex flex-col items-center">
                    <div class="w-12 h-12 rounded-full flex items-center justify-center text-xl {{ $step >= 2 ? 'bg-freshGreen text-white shadow-lg ring-4 ring-green-100' : 'bg-gray-200 text-gray-400' }}">
                        <i class="fa-solid {{ $isProduct ? 'fa-box' : 'fa-motorcycle' }}"></i>
                    </div>
                    <span class="text-xs font-bold mt-3 text-center {{ $step >= 2 ? 'text-gray-800' : 'text-gray-400' }}">
                        {!! $isProduct ? 'Pesanan<br>Dikemas' : 'Kurir<br>Jemput' !!}
                    </span>
                </div>
                
                <div class="relative z-10 flex flex-col items-center">
                    <div class="w-12 h-12 rounded-full flex items-center justify-center text-xl {{ $step >= 3 ? 'bg-freshGreen text-white shadow-lg ring-4 ring-green-100' : 'bg-gray-200 text-gray-400' }}">
                        <i class="fa-solid {{ $isProduct ? 'fa-truck-fast' : 'fa-hands-bubbles' }}"></i>
                    </div>
                    <span class="text-xs font-bold mt-3 text-center {{ $step >= 3 ? 'text-gray-800' : 'text-gray-400' }}">
                        {!! $isProduct ? 'Sedang<br>Dikirim' : 'Proses<br>Cuci' !!}
                    </span>
                </div>
                
                <div class="relative z-10 flex flex-col items-center"><div class="w-12 h-12 rounded-full flex items-center justify-center text-xl bg-gray-200 text-gray-400"><i class="fa-solid fa-box-open"></i></div><span class="text-xs font-bold mt-3 text-center text-gray-400">Selesai &<br>Diterima</span></div>
            </div>
            ...
            <div class="mt-10 bg-gray-50 rounded-2xl p-5 border border-gray-200 flex justify-between items-center">
                <div>
                    ...
                    <p class="text-lg font-extrabold text-shoeBlue">
                        @if($isProduct && $row->status == 'Menunggu Pickup') Sedang Dikemas
                        @elseif($isProduct && $row->status == 'Sedang Dicuci') Sedang Dikirim Kurir
                        @else {{ $row->status }}
                        @endif
                    </p>
                </div>
