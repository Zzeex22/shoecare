    ...
    <div class="grid md:grid-cols-2 gap-8">
        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden h-fit">
            ...
            <div class="p-6 bg-gray-50">
                ...
                <div class="flex justify-between items-center border-t border-gray-200 pt-4">
                    <span class="text-gray-500 font-bold text-sm uppercase">Total Harga</span>
        ...
        <div class="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
            ...
            <form method="POST" action="{{ route('toko.proses_checkout', $produk->id) }}" class="space-y-5">
                @csrf 
                <div>
                    <label class="text-xs font-bold text-gray-500 uppercase">1. No. WhatsApp Aktif</label>
                    <input type="number" name="no_hp" required placeholder="0812..." class="mt-1 w-full border-2 border-gray-200 p-3 rounded-xl bg-gray-50 focus:border-shoeBlue focus:bg-white outline-none transition">
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-500 uppercase">2. Metode Pembayaran</label>
                    <select name="metode_pembayaran" required class="mt-1 w-full border-2 border-gray-200 p-3 rounded-xl bg-gray-50 focus:border-shoeBlue focus:bg-white outline-none transition">
                        <option value="COD (Bayar di Tempat)">Bayar Tunai / COD</option>
                        <option value="QRIS / Transfer Bank">QRIS / Transfer Bank</option>
                    </select>
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-500 uppercase">3. Alamat Lengkap Pengiriman</label>
                    <textarea name="alamat_kirim" required rows="3" class="mt-1 w-full border-2 border-gray-200 p-3 rounded-xl bg-gray-50 focus:border-shoeBlue focus:bg-white outline-none transition" placeholder="Cth: Kost Biru, Kamar 4, Jl. Pancing..."></textarea>
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-500 uppercase">4. Pesan Tambahan (Opsional)</label>
                    <input type="text" name="catatan" placeholder="Cth: Titip di satpam ya bang" class="mt-1 w-full border-2 border-gray-200 p-3 rounded-xl bg-gray-50 focus:border-shoeBlue focus:bg-white outline-none transition">
                </div>
